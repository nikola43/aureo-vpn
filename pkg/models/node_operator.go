...
type NodeOperator struct {
	...
	TotalBandwidthKB       int64 `gorm:"default:0" json:"total_bandwidth_kb"`
...
type OperatorEarning struct {
	...
	// Earning details
	BandwidthKB     int64   `gorm:"not null" json:"bandwidth_kb"`
...
func (op *NodeOperator) UpdateStats(db *gorm.DB) error {
	// Calculate total earned (PAID earnings)
	var totalEarned float64
	db.Model(&OperatorEarning{}).
		Where("operator_id = ? AND status = ?", op.ID, "paid").
		Select("COALESCE(SUM(amount_usd), 0)").
		Scan(&totalEarned)

	// Calculate pending payout (CONFIRMED earnings ready for payout)
	// We can also include 'pending' if we want to show estimated earnings,
	// but for actual payout processing we usually only care about confirmed.
	// However, for the dashboard 'Pending Payout' usually implies what is accruing.
	// Let's sum both 'pending' and 'confirmed' for the UI, but ProcessPayouts should only pick confirmed.
	// Actually, strictly speaking, PendingPayout in the DB is used by ProcessPayouts.
	// So it MUST match what ProcessPayouts expects.
	// ProcessPayouts queries: pending_payout >= min.
	// So let's sum 'confirmed' only, to be safe.
...
func (op *NodeOperator) CalculateReputationScore(db *gorm.DB) float64 {
	...
	if op.TotalBandwidthKB > 1048576000 {
		score += 10.0
	} else if op.TotalBandwidthKB > 104857600 { // 100 GB
		score += 5.0
	}
